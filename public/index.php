...
try {
    ...
    $loader->registerNamespaces(
        [
            'Sms' => $config->application->providersDir,
        ]
    )->register();

    $di->setShared('smsProvider', function () use ($config) {
        $providerName = $config->smsProviderName;
        $smsProvider = new \Sms\Providers($providerName, $config->smsProviders->$providerName);
